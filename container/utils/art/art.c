...
void* art_search_approx(const art_tree *t, const unsigned char *key, int key_len) {
    ...
    while (n != NULL) {
        ...
        art_node **child = NULL;
        unsigned char min_char = 0xFF; // 初始化为最大可能的字符值

        switch (n->type) {
            case NODE4: {
                for (int i = 0; i < n->num_children; i++) {
                    unsigned char child_char = ((art_node4 *)n)->keys[i];
                    if (child_char >= key[depth] && (min_char == 0xFF || child_char < min_char)) {
                        min_char = child_char;
            ...
            case NODE16: {
                for (int i = 0; i < n->num_children; i++) {
                    unsigned char child_char = ((art_node16 *)n)->keys[i];
                    if (child_char >= key[depth] && (min_char == 0xFF || child_char < min_char)) {
                        min_char = child_char;

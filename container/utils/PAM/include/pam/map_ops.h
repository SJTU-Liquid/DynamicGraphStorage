...
struct map_ops : Seq {
  ...
  static node* find(node* b, const K& key) {
    ...
    ///@brief find the largest key that is less than or equal to the given key
    static node* find_approx(node* b, const K& key) {
      node* best = nullptr; 
      while (b) {
        if (Entry::comp(key, get_key(b))) {
          b = b->lc;
        } else {
          best = b; 
          b = b->rc; 
  ...
  static node* find_copy(node* b, const K& key) {
      while (b) {
  ...
  static inline void combine_values(node* a, ET& e, bool reverse, const BinaryOp& op) {
    ...
    if (reverse) Entry::set_val(re, std::move(op(Entry::get_val(e), Entry::get_val(re))));
    else Entry::set_val_rvalue(re, std::move(op(Entry::get_val(re), Entry::get_val(e))));
    Seq::set_entry_ref(a, re);
  }


  template <class BinaryOp>
  static inline void combine_values_ref(node* a, ET& e, bool reverse, const BinaryOp& op) {
    ET& re = Seq::get_entry(a);
	//std::cout << Entry::get_val(re) << " " << Entry::get_val(e) << std::endl;
    if (reverse) Entry::set_val_rvalue(re, std::move(op(Entry::get_val_ref(e), Entry::get_val_ref(re))));
    else Entry::set_val_rvalue(re, std::move(op(Entry::get_val(re), Entry::get_val(e))));
    Seq::set_entry_ref(a, re);
  }
  ...
  template <class BinaryOp>
  static inline void update_valuev_neo(node* a, const BinaryOp& op) {
    ET re = Seq::get_entry(a);
    Entry::set_val_rvalue(re, op(Entry::get_val(re)));
    Seq::set_entry(a, re);
  }
  ...
  static node* uniont(node* b1, node* b2, const BinaryOp op,
    ...
    if (bsts.removed) combine_values_ref(r, bsts.entry, true, op);
  ...
  template < class BinaryOp>
  static node* multi_update_sorted_neo(node* b, K* A, size_t n,
				   const BinaryOp& op, bool extra_ptr = false) {
	if (!b) return NULL;
    if (n == 0) return GC::inc_if(b, extra_ptr);
    bool copy = extra_ptr || (b->ref_cnt > 1);
    ...
    auto P = utils::fork<node*>(utils::do_parallel(Seq::size(b), n),
	       [&] () {return multi_update_sorted_neo(b->lc, A, mid, op, copy);},
	       [&] () {return multi_update_sorted_neo(b->rc, A+mid+dup,
						  n-mid-dup, op, copy);});

    node* r = GC::copy_if(b, copy, extra_ptr);
    if (dup) update_valuev_neo(r, op);
    return Seq::node_join(P.first, P.second, r);
  }

...
public:
  ...
  //   node_iterator(node* root) : current(root)  {
  //     pushLeft(root);
  //   }
  ...
  static typename Tree::node_iterator node_begin(M m) {
    return Tree::node_begin(m.get_root());
  }

  static typename Tree::node_iterator node_end() {
    return Tree::node_end();
  }
  ...
  // flatten all values to an output random access node_iterator
  ...
  // flatten all keys to an output random access node_iterator
  template <class outIter>
  ...
  static M multi_update_sorted(M m, Seq const &SS, Bin_Op f) {
    return M(Tree::multi_update_sorted(m.get_root(), const_cast<typename Seq::value_type*>(SS.data()), SS.size(), f));
  }

  template<class Seq, class Bin_Op>
  static M multi_update_sorted_neo(M m, Seq const &SS, Bin_Op f) {
    return M(Tree::multi_update_sorted_neo(m.get_root(), const_cast<typename Seq::value_type*>(SS.data()), SS.size(), f));
  }
  ...
  static M multi_insert_sorted(M m, Seq const &SS) {
    auto replace = [] (const V& a, const V& b) {return b;};
    return M(Tree::multi_insert_sorted(m.get_root(), const_cast<typename Seq::value_type*>(SS.data()),
				       SS.size(), replace));
  }
  ...
  maybe_V find_copy(const K& key) const {
  return node_to_val(Tree::find_copy(root, key));}

  /// find_approx returns the largest key less than or equal to the given key
  maybe_V find_approx(const K& key) const {
      return node_to_val(Tree::find_approx(root, key));
  }

  void acquire_version(node* root) {
    GC::inc(root);
  }
...
struct map_full_entry : entry {
  ...
  static inline key_t get_key(const entry_t& e) {return e.first;}
  static inline val_t get_val(const entry_t& e) {return e.second;}
  static inline const val_t& get_val_ref(const entry_t& e) {return e.second;}
  static inline void set_val(entry_t& e, const val_t& v) {e.second = v;}
  static inline void set_val_neo(entry_t& e, val_t&& v) {e.second = std::move(v);}
  static inline void set_val_rvalue(entry_t& e, val_t&& v) {e.second = std::move(v);}
};

...
#include "transaction/transaction_cow.hpp"
#include "utils/types.hpp"
#include "utils/config.hpp"
#include "container/container_cow.hpp"
...
public:
    ...
    uint64_t edge_count() const {
        auto tx = tm.get_read_transaction();
        uint64_t count = tx.container_impl->edge_count();
        tx.commit();
        return count;
    }
    ...
    public:
        explicit Snapshot(const TransactionManager<ContainerType>& tm)
            : m_tm(tm),
              ...
              m_num_edges(m_tm.container_impl->edge_count()) {}
    ...
    virtual std::unique_ptr<Snapshot> get_unique_snapshot() const {
        Snapshot tmp(tm);
        return std::make_unique<Snapshot>(std::move(tmp));
    }

    virtual std::shared_ptr<Snapshot> get_shared_snapshot() const {
        Snapshot tmp(tm);
